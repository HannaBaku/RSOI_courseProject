...
public class SqlOperation implements IOperation {
    ...
    @Override
    public Operation findByDate(String date, int id) throws SQLException {
        String str = "SELECT * FROM operation WHERE created_at='" + date + "' AND id_users=" + id;
        ArrayList<String[]> result = dbConnection.select(str);
        Operation operation = new Operation();
        DateTimeFormatter formatter = DateTimeFormatter.ofPattern("yyyy-MM-dd");
        for (String[] items: result){
